    ...
    myIo.on('connection', (socket) => {
        ...
        socket.on('activate-email', (data) => {
            ...
            let transporter = nodemailer.createTransport({
                service: 'Gmail',
                auth: {
                    user: 'followup30091994@gmail.com',
                    pass: 'Password@94'
                }
            });
            let mailOptions = {
                from: '"FollowUp" <Admin@FollowUp.com>', // sender address
                to: data.email, // list of receivers
                subject: 'Welcome to FollowUp', // Subject line
                html: `Hi ${data.firstName} ${data.lastName},<br><br>
                Welcome to the Follow Up App. It is used to find a suitable time to have a telephone conversation, web cast or meeting with participants in many time zones.<br>Please Click <a href="http://localhost:4200/activate?activateToken=${data.activateUserToken}" >here</a> to verify your email and continue with our sevices.<br><br> Warm Regards,<br>FollowUp Team` // html body
            };
        ...
        */
        ...
        socket.on('forgot-password', (data) => {
            ...
            let mailOptions = {
                ...
                html: `Hi,<br><br>If you are receiving this email, You have forgotten the password on Followup.<br>To reset the password Click the <a href="http://localhost:4200/reset?passwordToken=${data.resetPasswordToken}">link</a><br><b>The link will expire in 5 minutes</b><br><br>Warm Regards,<br>FollowUp Team` // html body
            };
            transporter.sendMail(mailOptions, (error, info) => {
                if (error) {
                    return console.log(error);
    ...
    eventEmitter.on('schedule-meeting', (data) => {
        ...
        let a = schedule.scheduleJob(data.meetingId, reminder, function () {
            ...
            let mailOptions = {
                ...
                html: `Hi ${data.userFullName},<br><br>The following meeting is about to start soon,<br><br><b>Title: </b>${data.title}<br><b>Venue: </b>${data.place}<br><b>Start: </b>${dateformat(data.start, "dddd, mmmm dS, yyyy, h:MM TT")}<br><b>End: </b>${dateformat(data.end, "dddd, mmmm dS, yyyy, h:MM TT")}<br><br>Warm Regards,<br>FollowUp Team` // html body
            };
            transporter.sendMail(mailOptions, (error, info) => {
                if (error) {
                    return console.log(error);
...
eventEmitter.on('meeting-update-email', (data) => {
    ...
    let mailOptions = {
        ...
        html: `Hi ${data.userFullName},<br><br>A meeting has been updated as below,<br><br><b>Title: </b>${data.title}<br><b>Venue: </b>${data.place}<br><b>Start: </b>${dateformat(data.start, "dddd, mmmm dS, yyyy, h:MM TT")}<br><b>End: </b>${dateformat(data.end, "dddd, mmmm dS, yyyy, h:MM TT")}<br><br>Warm Regards,<br>FollowUp Team` // html body
    };
    transporter.sendMail(mailOptions, (error, info) => {
        if (error) {
            return console.log(error);
...
eventEmitter.on('meeting-delete-email', (data) => {
    ...
    let mailOptions = {
        ...
        html: `Hi ${data.userFullName},<br><br>The following meeting has been deleted by admin,<br><br><b>Title: </b>${data.title}<br><b>Venue: </b>${data.place}<br><b>Start: </b>${dateformat(data.start, "dddd, mmmm dS, yyyy, h:MM TT")}<br><b>End: </b>${dateformat(data.end, "dddd, mmmm dS, yyyy, h:MM TT")}<br><br>Warm Regards,<br>FollowUp Team` // html body
    };
    transporter.sendMail(mailOptions, (error, info) => {
        if (error) {
            return console.log(error);
